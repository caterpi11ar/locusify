...
const content = {
  zh: {
    ...
    sections: [
      ...
      ['我们处理的信息', ['官网使用 Vercel Web Analytics 获取不使用 Cookie 的聚合访问指标，并使用 Google Analytics 4 了解页面访问、来源、设备和大致地区等网站使用情况。Google Analytics 可能使用 Cookie 或类似技术。托管与安全服务可能短期处理 IP 地址、User-Agent、请求时间和访问路径等标准请求日志。', '当你使用交互式应用并创建账户时，我们会处理电子邮箱和身份验证所需信息。照片和其中的位置数据仅在设备本地用于地图与路线回放。']],
      ['用途与法律依据', ['账户信息用于履行服务合同和保障账户安全；聚合分析和必要日志用于网站运行、安全防护与产品改进，依据我们的合法利益处理。我们不出售个人信息，也不将照片或位置数据用于广告画像。']],
      ['共享与保留', ['网站托管由 Vercel 提供，访问分析由 Vercel Web Analytics 与 Google Analytics 提供；这些服务商只在提供服务所需范围内处理数据。Google Analytics 数据按 Google 的服务条款和保留设置处理；账户数据保留至账户删除或不再提供服务；安全日志按托管服务商的有限保留周期处理。']],
  ...
  en: {
    ...
    sections: [
      ...
      ['Information we process', ['The website uses Vercel Web Analytics for aggregate, cookieless traffic metrics and Google Analytics 4 to understand page views, referrers, device categories, and approximate country or region. Google Analytics may use cookies or similar technologies. Hosting and security providers may temporarily process standard request logs such as IP address, user agent, request time, and path.', 'When you use the interactive app and create an account, we process your email address and authentication information. Photos and their location data remain on your device and are used locally for maps and route replay.']],
      ...
      ['Sharing and retention', ['Vercel provides website hosting, while Vercel Web Analytics and Google Analytics provide analytics services; these providers process data only as needed to provide their services. Google Analytics data is handled according to Google’s terms and retention settings. Account data is retained until account deletion or the service is discontinued; security logs follow the provider’s limited retention schedule; analytics are not used by Locusify to identify individuals.']],
