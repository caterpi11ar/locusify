...
每日任务的采集、Evidence Pack 和角色评审阶段严格只读，只有 Team=`approve` 且质量门禁=`pass` 后才进入条件执行阶段。O1 基线采集与 O2/O3 行动并行；每个采集和 Team 评审成功的每日 Run 至少执行一个有意义、低成本、非批量的平台动作，但不要求每天发帖。发布遵守滚动 7 天最多 3 条、相邻实际发布时间至少 48 小时、同日最多 1 条的频率门禁。缺少安全真实目标、参数或合规素材时必须 `defer` 并标记 `dailyActionStatus=blocked`，不得为满足数量刷量或发布低质量内容。数据不完整时输出 `partial`，不得用不完整快照覆盖趋势基线。“开始运营”不等于启用长期自动化；只有用户明确要求持续运行并确认后，才由配置管理 Skill 启用 desired Cron。
